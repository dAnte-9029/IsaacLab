...
Align the retention-aware scheduler's online strong-climb success signal with the frozen C2c episode-success contract, especially post-recovery tilt termination, then run a controlled fresh weights-only comparison from promoted C2c `model_550.pt`. Preserve the frozen C1/C2c/C3a grids and require two adjacent complete passes before C3b; do not promote or continue the failed optimizer as authority.
