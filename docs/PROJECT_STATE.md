...
PureRL C2 longitudinal engineering is implemented but has not yet been trained. Explicit C2a/C2b/C2c tasks sample level, climb, and descent paths with randomized heading, a 15--20 m level entry, a 20--30 m constant-slope segment, and an infinite level recovery. Reward progress is velocity along the three-dimensional path tangent; lateral and vertical normal velocity are penalized without a target-speed term. The actor observation remains the C1 555-value contract. Deterministic promotion grids contain 80/112/144 cases for C2a/C2b/C2c, while signed 10-degree cases remain diagnostic-only. Promotion requires two consecutive passing checkpoints at 100-iteration spacing after iteration 200 plus matching C1 retention evidence. Missing retention evidence fails closed.

Validation on 2026-08-10 passed 166 focused PureRL unit/contract tests, the existing 64-environment C1 CPU-native runtime gate (`1 passed in 13.63 s`), and the new six-environment C2a CPU-native runtime gate (`1 passed in 7.10 s`). The C2 runtime gate exercised fixed level/climb/descent schedules, path preview signs, orthonormal path bases, C1 level-reward equivalence, finite telemetry, partial resets, and repeated resets. No PPO training, wind injection, C3 geometry, direct-GPU qualification, or reward-weight tuning was performed. The compact real-flight profile remains evidence-only with `candidate_not_promoted`; it does not change environment defaults.
...
Select a qualified C1 checkpoint and start the first C2a CPU-native PPO run according to `docs/handoffs/2026-08-10-pure-rl-curriculum2.md`. Evaluate checkpoints every 100 iterations from iteration 200, run the matching C1 retention evaluation, and promote only after two consecutive combined passes. Do not start C2b or C2c from an unpromoted checkpoint. After C2c qualification, design C3 lateral/composite geometry with C1/C2 rehearsal. Keep wind and direct-GPU implicit drive in separate later qualification lanes.
