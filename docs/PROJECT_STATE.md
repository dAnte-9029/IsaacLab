...
Start C2b CPU-native training with 256 environments and 16 minibatches, warm-started cross-stage from promoted C2a `model_1575.pt`. Save every 25 iterations and apply the C2b grid plus C1 retention at the approved sample-equivalent cadence; do not start C2c from an unpromoted C2b checkpoint. After C2c qualification, design C3 lateral/composite geometry with C1/C2 rehearsal. Keep wind in a separate later qualification lane; the direct-GPU implicit training lane is rejected.
