"""Run the PureRL curriculum-1 fixed-action reward gate in a fresh process."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a bounded free-root CPU-native rollout with eight constant-action families and "
            "validate every PureRL curriculum-1 reward and termination telemetry channel."
        )
    )
...
def main() -> int:
    ...
    args.device = "cpu"
    print("[pure-rl-fixed-reward] launching Isaac Sim on CPU", flush=True)
    ...
    try:
        ...
        result = run_fixed_action_reward_job(
        ...
        print(
            f"[pure-rl-fixed-reward] wrote {args.summary} and {args.traces}; accepted={accepted}",
            flush=True,
        )
