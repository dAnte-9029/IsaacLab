"""Run one fresh-process CPU-native PureRL action step-response gate."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the CPU-native [frequency, rudder, left elevon, right elevon] "
            "PureRL action path. Run each mode in a fresh process."
        )
    )
    parser.add_argument("--mode", choices=("no_aero", "aero", "free_smoke"), required=True)
...
def main() -> int:
    ...
    args.device = "cpu"
    print(f"[pure-rl-action-step] launching Isaac Sim mode={args.mode}", flush=True)
    ...
    try:
        ...
        result = run_pure_rl_action_step_job(
            mode=args.mode,
            ...
            usd_dir=asset_cache,
            progress_callback=lambda message: print(f"[pure-rl-action-step] {message}", flush=True),
        )
        ...
        print(
            f"[pure-rl-action-step] wrote {args.summary} and {args.traces}; "
            f"accepted={summary['all_cases_accepted']}",
            flush=True,
        )
