...
def _broadcast_wrench_vec_to_strips(v: Tensor, *, N: int) -> Tensor:
    """Broadcast a vector (...,3) to per-strip (...,N,3) if needed."""
    if v.ndim >= 2 and v.shape[-2] == N and v.shape[-1] == 3:
        return v
    if v.shape[-1] != 3:
        raise ValueError("Expected a vector with last dimension 3.")
    return v.unsqueeze(-2).expand(*v.shape[:-1], N, 3)


def compute_aero_wrench_from_omega_alpha(
    omega_c: Tensor,
    alpha_c: Tensor,
    ...
    *,
    ...
    This is the same analytical model as :func:`compute_aero_wrench`, but driven directly
    by angular velocity/acceleration expressed in the co-rotating frame.

    Args:
        omega_c: Angular velocity ω_c in co-rotating frame, shape (..., 3) or (..., N, 3).
        alpha_c: Angular acceleration α_c in co-rotating frame, shape (..., 3) or (..., N, 3).
    ...
    if omega_c.shape[-1] != 3 or alpha_c.shape[-1] != 3:
        raise ValueError("omega_c/alpha_c must have last dimension 3.")

    device, dtype = omega_c.device, omega_c.dtype
    ...
    omega = _broadcast_wrench_vec_to_strips(omega_c, N=N)
    alpha = _broadcast_wrench_vec_to_strips(alpha_c, N=N)
    wx, wy, wz = omega.unbind(-1)  # (...,N)
    ax, _, az = alpha.unbind(-1)  # (...,N)

    # Per-strip geometry tensors (broadcast to (...,N)).
    x = geom.x_mid.view(*([1] * (wy.ndim - 1)), N)
    dx = geom.dx.view(*([1] * (wy.ndim - 1)), N)
    c = geom.c.view(*([1] * (wy.ndim - 1)), N)
    d_hat = geom.d_hat.view(*([1] * (wy.ndim - 1)), N)

    # ------------------------------------------------------------------
    # Translational velocity v_c per strip with optional forward-flight contribution (paper eq. (2.4)-(2.5)).
    # v_motion(x) = x * [0, ω_zc, -ω_yc]^T
    ...
    # paper eq. (2.6)
    _ = A_lift_max * torch.sin(2.0 * aoa)
...
def compute_aero_wrench(
    ...
    return compute_aero_wrench_from_omega_alpha(
        omega,
        alpha,
        geom,
        float(rho),
        include_wagner=include_wagner,
